...
@class IDULabel;
@class IDULabelLinkModel;
/**
 点击链点回调block

 @param linkModel 链点对应model
 */
typedef void (^IDULabelLinkModelBlock)(IDULabelLinkModel *linkModel);
...
@protocol IDULabelLinkDelegate <NSObject>
@optional
/**
 点击链点回调
...
IB_DESIGNABLE
/**
 * IDULabel 继承自 UILabel，其文本绘制基于NSAttributedString实现，同时增加了图文混排、富文本展示以及添加自定义点击链点并设置点击链点文本属性的功能。
 *
 *
 * IDULabel 与 UILabel 不同点：
 *
...
/**
 点击链点代理对象
 */
@property (readwrite, nonatomic, weak) id<IDULabelLinkDelegate> delegate;
...
/**
 点击链点model
 */
@interface IDULabelLinkModel : NSObject
...
@property (readonly, nonatomic, assign) CGRect imageRect;//链点图片Rect（相对于IDULabel坐标的rect）
